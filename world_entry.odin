...
entry_set :: proc(entry: ^World_Entry, value: $T, caller_loc := #caller_location) {
    comp_id := world_register(entry.world, T)
    v := value
    new_loc, loc_changed := world_set_entity_component(entry.world, entry.entity, comp_id, &v, entry.loc, caller_loc)
    if loc_changed {
        entry.loc = new_loc
        entry.arch = nil
    }
...
@(private)
entry_set_raw :: proc(entry: ^World_Entry, comp_id: Component_ID, data: rawptr, size: int, caller_loc := #caller_location) {
    new_loc, loc_changed := world_set_entity_component(entry.world, entry.entity, comp_id, data, entry.loc, caller_loc)
    if loc_changed {
        entry.loc = new_loc
        entry.arch = nil
    }
...
entry_remove :: proc(entry: ^World_Entry, $T: typeid, caller_loc := #caller_location) {
    comp_id, ok := world_get_component_id(entry.world, T)
    if !ok { return }
    new_loc, loc_changed := world_remove_entity_component(entry.world, entry.entity, comp_id, entry.loc, caller_loc)
    if loc_changed {
        entry.loc = new_loc
        entry.arch = nil
    }
...
@(private)
entry_remove_by_id :: proc(entry: ^World_Entry, comp_id: Component_ID, caller_loc := #caller_location) {
    new_loc, loc_changed := world_remove_entity_component(entry.world, entry.entity, comp_id, entry.loc, caller_loc)
    if loc_changed {
        entry.loc = new_loc
        entry.arch = nil

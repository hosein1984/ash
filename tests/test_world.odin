...
@(test)
test_world_spawn_with_components :: proc(t: ^testing.T) {
    world: ash.World
    ash.world_init(&world)
    defer ash.world_destroy(&world)
...
@(test)
test_spawn_component_order_independent :: proc(t: ^testing.T) {
    world: ash.World
    ash.world_init(&world)
    defer ash.world_destroy(&world)

    // Pre-register in a specific order
    ash.world_register(&world, Small)   // Small  = 0
    ash.world_register(&world, Medium)  // Medium = 1
    ash.world_register(&world, Large)   // Large  = 2

    // Spawn with components in DIFFERENT order than registration
    e := ash.world_spawn(&world, 
        Large{c = 3.14159},     // ID 2
        Small{a = 42},                       // ID 0
        Medium{b = 12345},                   // ID 1
    )
    ...
    small  := ash.entry_get(entry, Small)
    medium := ash.entry_get(entry, Medium)
    large  := ash.entry_get(entry, Large)

    testing.expect_value(t, small.a, 42)
    testing.expect_value(t, medium.b, 12345)
    testing.expect_value(t, large.c, 3.14159)
}

@(test)
test_spawn_batch_component_order_independent :: proc(t: ^testing.T) {
    world: ash.World
    ash.world_init(&world)
    defer ash.world_destroy(&world)

    ash.world_register(&world, Small)
    ash.world_register(&world, Medium)
    ash.world_register(&world, Large)

    // Spawn batch with reversed component order
    entities := ash.world_spawn_batch(
        &world,
        3,
        Large{c = 2.718},
        Medium{b = 999},
    ...
    defer delete(entities, allocator = context.allocator)
    ...
    for e in entities {
        ...
        testing.expect_value(t, small.a, 7)
        testing.expect_value(t, medium.b, 999)
        testing.expect_value(t, large.c, 2.718)
    }
}

@(test)
test_spawn_mixed_registration_order :: proc(t: ^testing.T) {
    world: ash.World
    ash.world_init(&world)
    defer ash.world_destroy(&world)

    // Register Large first, then Small
    ash.world_register(&world, Large)  // ID 0
    ash.world_register(&world, Small)  // ID 1

    // Spawn with Small first - comp_ids would be [1, 0], sorted to [0, 1]
    e := ash.world_spawn(&world,
        Small{a = 100},  // ID 1
        Large{c = 1.5},  // ID 0
    )
    ...
    testing.expect_value(t, small.a, 100)
    testing.expect_value(t, large.c, 1.5)
}
